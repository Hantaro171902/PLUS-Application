...
const Post = ({ post }) => {
  ...
  const { like, setLiked } = useState(post.like.includes(user?._id) || false);
  ...
  const likeOrDislikeHandler = async (postId) => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:8000/api/v1/post/delete/${postId}/${action}`,
  ...
  const commentHandler = async () => {
    try {
      const res = await axios.post(
        `http://localhost:8000/api/v1/user/post/${post._id}/comment}`,
        { text },
        {
          headers: {
  ...
  const deletePostHandler = async () => {
    try {
      const res = await axios.delete(
        `http://localhost:8000/api/v1/user/post/${post._id}`,
        { withCredentials: true }
      );
      if (res.data.success) {
